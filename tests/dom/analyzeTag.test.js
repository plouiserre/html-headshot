...
test('Analyze simple Tag without anything', ()=>{
    ...
    const expected = {tagName : "title", completeTag : html, content: "Liste des Pokémon de la première génération — Poképédia", cssClass:'', cssId : '', "contentOnlyText":true};
...
test('Analyze simple Tag with only one class css', async ()=>{
    ...
    const expected = {tagName : 'ul', completeTag: html, content:'', cssClass : 'vector-menu-content-list', cssId : '', contentOnlyText : true};
    expect(JSON.stringify(expected)).toBe(JSON.stringify(result));
});
...
test('Analyze simple Tag with one class css and texte', async ()=>{
    ...
    const expected = {tagName : 'span', completeTag : html , content:'Outils personnels', cssClass : 'vector-dropdown-label-text', cssId : '', contentOnlyText : true};
    expect(JSON.stringify(expected)).toBe(JSON.stringify(result));
});
...
test('Analyze Tags with one attribut, one class css and texte', async ()=>{
    ...
    const expected = {tagName : 'div', completeTag : html, content:'déplacer vers la barre latérale', cssClass : 'vector-pinned-container', cssId:'vector-main-menu-pinned-container',  contentOnlyText : true};
    expect(JSON.stringify(expected)).toBe(JSON.stringify(result));
});
...
test('Analyze Tags with one attribut, two class css, and many sub element with texte', async ()=>{
    ...
    const expected = {tagName : 'li', completeTag : html, content:'<a href="/Liste_des_Pok%C3%A9mon_de_la_premi%C3%A8re_g%C3%A9n%C3%A9ration"><span>Lire</span></a>', cssClass : 'selected vector-tab-noicon mw-list-item', cssId:'ca-view',  contentOnlyText : false};
    expect(JSON.stringify(expected)).toBe(JSON.stringify(result));
});
...
test('Analyze tag with one space before the opens', async()=>{
    ...
    const expected = {tagName : 'div', completeTag :'<div class="vector-settings" id="p-dock-bottom"><ul></ul></div>', content:'<ul></ul>', cssClass : 'vector-settings', cssId:'p-dock-bottom',  contentOnlyText : false};
    expect(JSON.stringify(expected)).toBe(JSON.stringify(result));
});
...
test('Analyze tag with many levels with same closedTags', async()=>{
    ...
    const expected = {tagName : 'div', completeTag : html, content:contentHtml, cssClass : 'vector-column-start', cssId:'',  contentOnlyText : false};
    expect(JSON.stringify(expected)).toBe(JSON.stringify(result));
});
...
test('Analyze tag when in content you have tag inside text', async()=>{
    ...
    const expected = {tagName : 'li', completeTag : html, content:'Le contenu est disponible sous licence <a href="/Pok%C3%A9p%C3%A9dia:Copyrights" title="Poképédia:Copyrights">Paternité-Pas d\'Utilisation Commerciale-Partage des Conditions Initiales à l\'Identique 3.0</a> sauf mention contraire.', cssClass : '', cssId:'footer-info-copyright',  contentOnlyText : true};
    expect(JSON.stringify(expected)).toBe(JSON.stringify(result));
});