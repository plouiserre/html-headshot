...
test('can analyze simple Tag', ()=>{
    const tag = 'title';
    const html = 'Liste des Pokémon de la première génération — Poképédia</title><script>(function(){var className="client-js vector-feature-language-in-header';
    const extractTag = new ExtractTag();
    const result = extractTag.extract(tag, html);

    const resultWaiting = '<title>Liste des Pokémon de la première génération — Poképédia</title>'
    expect(resultWaiting).toBe(result);
});


test('can analyze hard Tag', ()=>{
    const tag = 'span';
    const html = ' class="vector-dropdown-label-text">Menu principal</span> </label>';
    const extractTag = new ExtractTag();
    const result = extractTag.extract(tag, html);
...
//when the dev of this feature is finished I will delete the second test and keep only this because more complexe
test('can analyze many Tags', ()=>{
    const tag = 'header';
    const html = ' class="mw-body-header vector-page-titlebar"><h1 id="firstHeading" class="firstHeading mw-first-heading"><span class="mw-page-title-main">Liste des Pokémon de la première génération</span></h1></header> <div class="vector-page-toolbar"> <div class="vector-page';
    const extractTag = new ExtractTag();
    const result = extractTag.extract(tag, html);
