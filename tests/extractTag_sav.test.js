...
//     const resultWaiting = {html : '<title>Liste des Pokémon de la première génération — Poképédia</title>', extraction : true, simpleTag: 'title', htmlToDelete : '<title>Liste des Pokémon de la première génération — Poképédia</title>'};
...
//     const resultWaiting = {html : '<span class="vector-dropdown-label-text">Menu principal</span>', extraction : true, simpleTag: 'span', htmlToDelete : '<span class="vector-dropdown-label-text">Menu principal</span>'};
//     expect(JSON.stringify(result)).toBe(JSON.stringify(resultWaiting));
// });
...
//     const resultWaiting = {html :'<header class="mw-body-header vector-page-titlebar"><h1 id="firstHeading" class="firstHeading mw-first-heading"><span class="mw-page-title-main">Liste des Pokémon de la première génération</span></h1></header>', extraction : true, simpleTag: 'header', htmlToDelete : '<header class="mw-body-header vector-page-titlebar"><h1 id="firstHeading" class="firstHeading mw-first-heading"><span class="mw-page-title-main">Liste des Pokémon de la première génération</span></h1></header>'};
//     expect(JSON.stringify(result)).toBe(JSON.stringify(resultWaiting));
// });
...
//     const htmlWaiting = '<div id="p-associated-pages" class="vector-menu vector-menu-tabs mw-portlet mw-portlet-associated-pages"><div class="vector-menu-content"><ul class="vector-menu-content-list"><li id="ca-nstab-main" class="selected vector-tab-noicon mw-list-item"><a href="/Liste_des_Pok%C3%A9mon_de_la_premi%C3%A8re_g%C3%A9n%C3%A9ration" title="Voir le contenu de la page [c]" accesskey="c"><span>Page</span></a></li><li id="ca-talk" class="new vector-tab-noicon mw-list-item"><a href="/index.php?title=Discussion:Liste_des_Pok%C3%A9mon_de_la_premi%C3%A8re_g%C3%A9n%C3%A9ration&amp;action=edit&amp;redlink=1" rel="discussion" title="Discussion au sujet de cette page de contenu (page inexistante) [t]" accesskey="t"><span>Discussion</span></a></li></ul></div></div>';
//     const resultWaiting = {html : htmlWaiting, extraction : true, simpleTag: 'div', htmlToDelete : htmlWaiting};
//     expect(JSON.stringify(result)).toBe(JSON.stringify(resultWaiting));
// });
...
//     const resultWaiting = {html : '<!DOCTYPE html>', extraction : false, simpleTag: '!DOCTYPE', htmlToDelete : '<!DOCTYPE html>'};
//     expect(JSON.stringify(result)).toBe(JSON.stringify(resultWaiting));
// })
...
//     const htmlWaiting = '<meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=yes, minimum-scale=0.25, maximum-scale=5.0">';
//     const resultWaiting = {html : htmlWaiting, extraction : false, simpleTag: 'meta', htmlToDelete : htmlWaiting};
//     expect(JSON.stringify(result)).toBe(JSON.stringify(resultWaiting));
// })
...
//     const htmlWaiting = '<link rel="stylesheet" href="/load.php?lang=fr&amp;modules=site.styles&amp;only=styles&amp;skin=vector-2022">';
//     const resultWaiting = {html : htmlWaiting , extraction : false, simpleTag: 'link', htmlToDelete : htmlWaiting};
//     expect(JSON.stringify(result)).toBe(JSON.stringify(resultWaiting));
// })
...
//     const htmlWaiting = '<input class="cdx-text-input__input" type="search" name="search" placeholder="Rechercher sur Poképédia" aria-label="Rechercher sur Poképédia" autocapitalize="sentences" title="Rechercher sur Poképédia [f]" accesskey="f" id="searchInput" >';
//     const resultWaiting = {html : htmlWaiting, extraction : false, simpleTag: 'input', htmlToDelete : htmlWaiting};
//     expect(JSON.stringify(result)).toBe(JSON.stringify(resultWaiting));
// })
...
//     const htmlWaiting = '<img alt="0001" src="/images/d/db/Miniature_0001_EV.png" decoding="async" width="256" height="256" class="mw-file-element" />';
//     const resultWaiting = {html : htmlWaiting, extraction : false, simpleTag: 'img', htmlToDelete : htmlWaiting};
//     expect(JSON.stringify(result)).toBe(JSON.stringify(resultWaiting));
// }) 
...
//     const resultWaiting = {html : '', extraction : false, simpleTag: 'script', htmlToDelete : ''};
//     expect(JSON.stringify(result)).toBe(JSON.stringify(resultWaiting));
// });
