...
test('Find tag with class vector-menu-content-list', ()=>{
    ...
    const result = search.find({identifier :'vector-menu-content-list', type:'class'}); 
...
test('Find tag with tagName title', ()=>{
    ...
    const result = search.find({identifier :'ul', type:'tagName'}); 
...
test('Find tag with id pt-createaccount located with many class', ()=>{
    ...
    const result = search.find({identifier :'pt-createaccount', type :'id'}); 
...
test('Find tags with tagName span', ()=>{
    ...
    const results = search.find({identifier :'span', type:'tagName'}); 
...
test('Find tags with all classNames user-links-collapsible-item mw-list-item', ()=>{
    ...
    const results = search.find({identifier :'user-links-collapsible-item mw-list-item', type:'class'}); 
...
test('Find tags with classNames vector-icon', ()=>{
    ...
    const results = search.find({identifier :'vector-icon', type:'class'}); 
...
test('Find no tags with className vector', ()=>{
    ...
    expect(()=>search.find({identifier :'vector', type:'class'})).toThrow(`La classe vector n'est attribuée à aucun élément`);
...
test('Find no tags title', ()=>{
    ...
    expect(()=>search.find({identifier :'title', type:'tagName'})).toThrow(`La balise title n'est attribuée à aucun élément`);
});

test('Find no tags with id main', ()=>{
    const search = new Search(domResults);

    expect(()=>search.find({identifier :'main', type:'id'})).toThrow(`L'id main n'est attribuée à aucun élément`);
});